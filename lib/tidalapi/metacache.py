...
class MetaCache:
    ...
    def connect(self):
        self.path = os.path.join(settings.cacheDir, CONST.metaCacheFile)
        try:
    ...
    def fetch(self, item_type, item_id):
        item = None
        try:
            if not self.dbcon:
                self.connect()
    ...
    def insertAlbumJson(self, json_obj):
        if settings.cacheAlbums and json_obj.get('id') and json_obj.get('releaseDate'):
